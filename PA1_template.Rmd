...
hist(totdat$total_steps, breaks = 15, xlab = "Total steps", main = "Histogram over total steps for each day")
...
avgdat <- dat %>% group_by(interval) %>% summarize(avg_steps = mean(steps))
